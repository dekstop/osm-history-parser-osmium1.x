...
protected:
    std::unordered_map<osm_user_id_t, time_t> first_edit;
...
public:
    ...
    void node(const shared_ptr<Osmium::OSM::Node const> &node) {
        ...
        time_t ts = node->timestamp();
        std::unordered_map<osm_user_id_t, time_t>::const_iterator it = first_edit.find(uid);
        if (it == first_edit.end() || it->second > ts) {
            first_edit[uid] = ts;
        }
    }
    ...
    void after_nodes() const {
        ...
        for (auto it = first_edit.cbegin(); it != first_edit.cend(); ++it) {
            m_outfile <<  
                it->first << "\t" <<
                it->second << std::endl;
        }
